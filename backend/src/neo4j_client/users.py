"""Neo4j Client for Managing Users"""
...
class Users(neo4j_client.base.BaseClient):
    async def signup(self, username: str, hashed_password: str):
        async with self._neo4j_driver.session() as session:
            response = await session.run(
                f"MATCH (u:User) WHERE u.username='{username}' RETURN u.username"
            )
    ...
    async def get_user_password(self, username: str) -> str:
        async with self._neo4j_driver.session() as session:
            response = await session.run(
                f"MATCH(u:User) WHERE u.username='{username}' RETURN u.password as password"
            )
            try:
    ...
    async def update_user(self, username: str, new_password: str):
        async with self._neo4j_driver.session() as session:
            await session.run(
                f"MATCH(u:User) WHERE u.username='{username}' SET u.password = '{new_password}'"
            )
    ...
    async def get_user_last_updated_timestamp(self, username: str) -> int:
        async with self._neo4j_driver.session() as session:
            response = await session.run(
                f"MATCH(u:User) WHERE u.username='{username}' RETURN u.lastUpdated"
            )
            result = await response.single(strict=True)
            return int(result[0])

    async def set_user_last_updated_timestamp(self, username: str):
        async with self._neo4j_driver.session() as session:
            await session.run(
                f"MATCH(u:User) WHERE u.username='{username}'"
                f" SET u.lastUpdated = '{int(pendulum.now().timestamp())}'"
            )
